...
const ServiceDetails = () => {
  ...
  const handleBooking = () => {
    alert(`Booking requested for: ${service?.service_name}`);
  };

  const handleReviewSubmit = async () => {
    if (!rating || !comment) return alert("Add rating & comment");
  ...
  return (
    ...
    >
      ...
      <p className="text-xl text-purple-400 mb-4">{service.service_category}</p>
      ...
      <motion.button
        ...
        className="w-full py-4 rounded-xl btn-primary-gradient text-white font-bold text-xl shadow-lg"
      >
        Book This Service
      </motion.button>
